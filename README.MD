...
3. Set up PostgreSQL database and update connection details in `Main.ts` if needed.

4. Run the database setup (ensure tables are created):
   - The app creates the `users` table automatically.
   - Manually create the `api_keys` table:
     ```sql
     CREATE TABLE api_keys (
       id SERIAL PRIMARY KEY,
       user_id INTEGER REFERENCES users(id),
       hashApi VARCHAR(255) NOT NULL,
